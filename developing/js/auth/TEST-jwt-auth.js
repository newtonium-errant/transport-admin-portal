// JWT Authentication Helper - TEST VERSION
// For Testing Branch pages in developing/ folder
// Version: 1.0.0-TEST
//
// Differences from production:
// - Uses TEST- prefixed endpoints (TEST-user-login, TEST-refresh-token, etc.)
// - Redirects to TEST-dashboard.html instead of dashboard.html

// API Configuration - TEST endpoints
const JWT_API_ENDPOINTS = {
    LOGIN: 'https://webhook-processor-production-3bb8.up.railway.app/webhook/TEST-user-login',
    CHANGE_PASSWORD: 'https://webhook-processor-production-3bb8.up.railway.app/webhook/TEST-change-password',
    REFRESH_TOKEN: 'https://webhook-processor-production-3bb8.up.railway.app/webhook/TEST-refresh-token'
};

// Check if user is authenticated (call this on page load)
async function requireAuth(redirectUrl = 'TEST-dashboard.html') {
...
function logout() {
    ...
    // Redirect to TEST-dashboard.html (which shows login screen)
...
console.log('JWT Authentication Helper (TEST VERSION) loaded');
