...
class AppointmentsPage {
    ...
    async init() {
        ...
        if (!(await requireAuth('TEST-dashboard.html'))) {
            return;
        }
    ...
    // Note: Navigation is handled by TEST-navigation.js component
    initHeader() {
        // User info display is handled by TEST-navigation.js
        // buildNavigation() and setupMobileMenu() are handled by TEST-navigation.js
    }
    ...
    async loadInitialData() {
        ...
        try {
            ...
            const response = await authenticatedFetch(
                'https://webhook-processor-production-3bb8.up.railway.app/webhook/TEST-get-appointments-page-data'
            );
    ...
    async loadAppointments(loadAllHistorical = false) {
        ...
        try {
            const response = await authenticatedFetch(
                'https://webhook-processor-production-3bb8.up.railway.app/webhook/TEST-get-all-appointments'
            );
    ...
    async confirmCancellation(appointmentId) {
        ...
        try {
            ...
            const response = await authenticatedFetch(
                'https://webhook-processor-production-3bb8.up.railway.app/webhook/TEST-cancel-appointment',
                {
                    method: 'POST',
    ...
    async deleteAppointment(appointmentId, deleteType = 'soft') {
        ...
        try {
            ...
            const endpoint = deleteType === 'hard'
                ? 'https://webhook-processor-production-3bb8.up.railway.app/webhook/TEST-hard-delete-appointment'
                : 'https://webhook-processor-production-3bb8.up.railway.app/webhook/TEST-soft-delete-appointment';
    ...
    async unarchiveAppointment(appointmentId) {
        ...
        try {
            ...
            const response = await authenticatedFetch(
                'https://webhook-processor-production-3bb8.up.railway.app/webhook/TEST-unarchive-appointment',
                {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
        ...
        if (typeof appointmentModalInstance !== 'undefined' && appointmentModalInstance) {
            ...
            try {
                ...
                if (mode === 'edit') {
                    ...
                    endpoint = 'https://webhook-processor-production-3bb8.up.railway.app/webhook/TEST-update-appointment';
                    payload = appointmentData;
                } else {
                    // Add endpoint - uses pre-calculated travel times (simplified payload)
                    endpoint = 'https://webhook-processor-production-3bb8.up.railway.app/webhook/TEST-save-appointment';
