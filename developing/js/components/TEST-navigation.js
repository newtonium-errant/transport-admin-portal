/**
 * RRTS Navigation Component - TEST VERSION
 * Provides consistent RBAC-based navigation across all TEST pages
 * Version: 1.0.0
 *
 * NOTE: This is the TEST version with TEST- prefixed page names
 */

// Standard navigation items with RBAC roles - TEST VERSION
const NAV_ITEMS = [
    { page: 'TEST-dashboard', label: 'Dashboard', icon: 'bi-speedometer2', roles: ['admin', 'supervisor', 'booking_agent'] },
    { page: 'TEST-appointments-sl', label: 'Appointments', icon: 'bi-calendar-check', roles: ['admin', 'supervisor', 'booking_agent'] },
    { page: 'TEST-appointments-bulk-add', label: 'Bulk Add', icon: 'bi-calendar-plus', roles: ['admin', 'supervisor', 'booking_agent'] },
    { page: 'TEST-clients-sl', label: 'Clients', icon: 'bi-people', roles: ['admin', 'supervisor', 'booking_agent'] },
    { page: 'TEST-driver-management', label: 'Drivers', icon: 'bi-truck', roles: ['admin', 'supervisor'] },
    { page: 'TEST-operations', label: 'Operations', icon: 'bi-clipboard-data', roles: ['admin', 'supervisor'] },
    { page: 'TEST-finance', label: 'Finance', icon: 'bi-currency-dollar', roles: ['admin', 'supervisor'] },
    { page: 'TEST-admin', label: 'Admin', icon: 'bi-gear', roles: ['admin'] }
];
...
function initializeNavigation(options = {}) {
    ...
    const currentPage = window.location.pathname.split('/').pop().replace('.html', '') || 'TEST-dashboard';
...
/**
 * Standard logout function - clears all tokens and redirects to TEST-dashboard
 */
function logout() {
