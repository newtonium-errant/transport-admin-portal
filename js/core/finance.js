...
(function() {
    ...
    console.log('[Finance] Loading Finance Dashboard v5 - Complete - 5.0.0');
    ...
    async function loadAppConfig() {
        try {
            ...
            // Fetch from API
            const response = await APIClient.get('/get-app-config');
    ...
    async function loadAllData() {
        try {
            ...
            const [appointmentsData, invoicesData, driversData, usersData, clientsData] = await Promise.all([
                APIClient.get('/get-finance-appointments').catch(err => {
                ...
                APIClient.get('/get-all-drivers').catch(err => {
                    console.warn('[Finance v5] Error loading drivers:', err);
                    return { data: [] };
                }),
                APIClient.get('/get-all-users').catch(err => {
                    console.warn('[Finance v5] Error loading users:', err);
                    return { data: [] };
                }),
                APIClient.get('/get-all-clients').catch(err => {
            ...
            console.log('[Finance] Invoices parsed:', allInvoices.length, 'invoices', allInvoices);
            ...
            console.log(`[Finance] Loaded: ${allAppointments.length} appointments, ${allInvoices.length} invoices, ${drivers.length} drivers, ${users.length} users, ${clients.length} clients`);
    ...
    async function bulkMarkReady() {
        ...
        try {
            ...
            await APIClient.post('/mark-appointment-ready', { appointmentIds });
    ...
    async function createInvoice(knumber, appointmentIds) {
        ...
        const response = await APIClient.post('/create-invoice', payload);
    ...
    async function voidInvoice(invoiceId) {
        ...
        try {
            await APIClient.post('/void-invoice', { invoiceId });
    ...
    async function markAgentPaid(agentId) {
        ...
        try {
            ...
            await APIClient.post('/mark-agent-paid', {
                appointmentIds,
