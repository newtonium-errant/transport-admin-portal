...
class ClientModal {
                ...
                <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                          <!-- Incomplete Profile Alert (hidden by default) -->
                          <div id="incompleteProfileAlert" class="alert alert-warning d-flex align-items-start mb-3" role="alert" style="display: none !important;">
                              <i class="bi bi-exclamation-triangle-fill me-2 mt-1"></i>
                              <div>
                                  <strong>Incomplete Profile</strong> — Please fill in the highlighted fields to complete this client's profile.
                              </div>
                          </div>
                          <form id="clientQuickEditForm" onsubmit="event.preventDefault(); clientModalInstance.save();">
                            <!-- Client Name & K Number -->
                            <div class="row">
                                <div class="col-md-8">
                                    <div class="mb-3">
                                ...
                                <div class="col-md-4">
                                    <div class="mb-3">
                                        <label for="clientKNumber" class="form-label fw-bold">
                                            K Number
                                            <i class="bi bi-hash ms-1"></i>
                                        </label>
                                        <input type="text" class="form-control" id="clientKNumber" readonly>
                                        <small class="text-muted" id="clientKNumberHelp"></small>
                                    </div>
                                </div>
                            </div>
    ...
    populateForm(client) {
        ...
        // K Number field
        const kNumberField = document.getElementById('clientKNumber');
        const kNumberHelp = document.getElementById('clientKNumberHelp');
        const isAutoGenerated = client.knumber && /^T/.test(client.knumber);
        const isIncomplete = client.profile_status === 'incomplete';

        kNumberField.value = client.knumber || '';

        if (isIncomplete && isAutoGenerated) {
            // Editable for incomplete clients with auto-generated K numbers
            kNumberField.readOnly = false;
            kNumberField.classList.remove('bg-light');
            kNumberHelp.textContent = 'Temporary \u2014 please update with real K number';
            kNumberHelp.style.color = '#856404';
        } else {
            // Read-only for complete clients or real K numbers
            kNumberField.readOnly = true;
            kNumberField.classList.add('bg-light');
            kNumberHelp.textContent = '';
        }
        ...
        // Handle incomplete profile indicator
        this.highlightIncompleteFields(client);
    }

    /**
     * Check profile completeness and highlight missing fields
     * @param {Object} client - Client data
     */
    highlightIncompleteFields(client) {
        const alert = document.getElementById('incompleteProfileAlert');
        const isIncomplete = client.profile_status === 'incomplete';

        // Show/hide the alert banner
        if (alert) {
            alert.style.display = isIncomplete ? 'flex' : 'none';
            alert.style.setProperty('display', isIncomplete ? 'flex' : 'none', 'important');
        }

        // Define required fields for a complete profile and their form element IDs
        // K number is "missing" if it's a T-prefixed temp placeholder
        const isAutoGeneratedK = client.knumber && /^T/.test(client.knumber);
        const requiredFieldMap = {
            'clientKNumber': isAutoGeneratedK ? null : client.knumber,
        ...
        document.querySelectorAll('#clientQuickEditModal .incomplete-highlight').forEach(el => {
            el.classList.remove('incomplete-highlight');
            el.style.borderColor = '';
            el.style.backgroundColor = '';
        });

        // Remove any previously injected "Required" badges
        document.querySelectorAll('#clientQuickEditModal .incomplete-required-badge').forEach(el => {
            el.remove();
        });
        ...
        Object.entries(requiredFieldMap).forEach(([fieldId, value]) => {
            if (!value || (typeof value === 'string' && !value.trim())) {
                const field = document.getElementById(fieldId);
                if (field) {
                    field.classList.add('incomplete-highlight');
                    field.style.borderColor = '#f0ad4e';
                    field.style.backgroundColor = '#fffbf0';

                    // Add "Required" badge to the label
                    const label = field.closest('.mb-3')?.querySelector('.form-label');
                    if (label && !label.querySelector('.incomplete-required-badge')) {
                        const badge = document.createElement('span');
                        badge.className = 'incomplete-required-badge badge bg-warning text-dark ms-2';
    ...
    async save() {
        ...
        try {
            ...
            // Read K number from form (may have been updated from auto-generated)
            const editedKNumber = document.getElementById('clientKNumber').value.trim() || this.client.knumber;
            const originalKNumber = this.client.knumber;

            const requestBody = {
                kNumber: editedKNumber,
                originalKNumber: editedKNumber !== originalKNumber ? originalKNumber : undefined,
                clientData: {
            ...
            // T-prefixed temp placeholders don't count as "real" K numbers
            const cd = requestBody.clientData;
            const hasRealKNumber = requestBody.kNumber && !/^T/.test(requestBody.kNumber);
            const isComplete = !!(
                hasRealKNumber &&
                cd.firstName && cd.lastName &&
                cd.phone && cd.email &&
                cd.civicAddress && cd.city && cd.province && cd.postalCode &&
                cd.emergencyContactName && cd.emergencyContactNumber
            );
